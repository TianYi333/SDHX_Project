...
extern volatile uint32_t Transmit_Tickstart;
extern volatile uint32_t Transmit_Ticknow;
extern volatile uint32_t Transmit_Timeout_Cnt;
...
void CAN_Config_Ready(void)
{
	...
	CAN_FilterInitStructure.FilterMode = 	CAN_FILTERMODE_IDLIST;	//CAN_FILTERMODE_IDMASK;
	...
	CAN_FilterInitStructure.FilterIdHigh = 0x00A0 << 5; 
	CAN_FilterInitStructure.FilterIdLow = 0x000A1 << 5;
	CAN_FilterInitStructure.FilterMaskIdHigh = 0X00A2 << 5;
	CAN_FilterInitStructure.FilterMaskIdLow = 0X00AA << 5;
		...
		if(CAN2_RxHeader.StdId == 0xA0)
		{
			if (Status_Flag == 0) //存储要接收的数据长度
			{
				if (can2_recv_buf[0] == 0xAA && can2_recv_buf[1] == 0xAA)
				{
					...
					Transmit_Timeout_Cnt = 0;
					Transmit_Tickstart = HAL_GetTick();
				}
			}
		}
