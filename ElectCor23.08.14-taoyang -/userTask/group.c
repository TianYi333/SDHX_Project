				...
				case MOVE_GROUP_ADVANCING:
					...
					if(Structset_Framemove_Forbiden1_3.Setting_Para == 1)//移架允许
					{
								...
								i_DcolumnAgainFrequency =Structset_Framemove_DcolumnAgainFrequency13_1.Setting_Para;
									...
									Faction_group_UP(MOVE_COLUMN_DOWN,MOVE_FUEL_SPRAY,0,1);//上位机上报，降柱喷雾开始
...
//													Faction_group_UP(MOVE_SIDE_PLATE_STRETCH,0,0,1);//上位机上报												
//											}
...
//													Faction_group_UP(MOVE_SIDE_PLATE_STRETCH,0,0,0);//上位机上报
...
//											}
									...
									Faction_group_UP(MOVE_COLUMN_DOWN,0,0,0);//上位机上报,降柱结束
									i=0;
									contorl_Valve = 0;
									...
									Faction_group_UP(MOVE_HYDRAULICFRAME_ADVANCING,0,0,1);//上位机上报,移驾开始
...
//											}
									...
									Faction_group_UP(MOVE_HYDRAULICFRAME_ADVANCING,0,0,0);//上位机上报,移驾结束
									...
									Faction_group_UP(MOVE_COLUMN_UP,0,0,1);//上位机上报,移架开始
									...
									Faction_group_UP(MOVE_COLUMN_UP,MOVE_FUEL_SPRAY,0,0);//上位机上报,移驾结束
				...
				case MOVE_GROUP_CHUTE_PUSH:
						if(Structset_ChutePush_Forbiden1_3.Setting_Para == 1)
						{
							...
							if(_sensor_distance.sensor_value + Structset16_ChutePush_aimDistance3_1.Setting_Para >= 700)
							{
								Distance_M = 700;
							}
							else if(_sensor_distance.sensor_value + Structset16_ChutePush_aimDistance3_1.Setting_Para <700)
							{
									Distance_M = _sensor_distance.sensor_value + Structset16_ChutePush_aimDistance3_1.Setting_Para;
							}
								Faction_group_UP(MOVE_CHUTE_PUSH,0,0,1);//上位机上报,推溜开始	
								...
								Faction_group_UP(MOVE_CHUTE_PUSH,0,0,0);//上位机上报,推溜开始
						}
				break;
...
void Faction_group_UP(uint8_t upper_num1,uint8_t upper_num2,uint8_t upper_num3,uint8_t State)
{
	...
	uint8_t upper_message[7] = {0};
	  ...
	  upper_message[2] = upper_num1;//动作号
	  upper_message[3] = State;
	  upper_message[4] = 0;//邻架控制动作
